...
def get_uptime():
    try:
        with open(config.PROC_UPTIME_PATH) as f:
            uptime_str = f.read()
...
def get_meminfo():
    try: 
        with open(config.PROC_MEMINFO_PATH) as f:
            lines = f.readlines()
    except FileNotFoundError:
        return None
    meminfo = {}
...
def get_cpu_usage():
    try:
        with open(config.PROC_STAT_PATH) as f:
            lines1 = f.readlines()
    except FileNotFoundError:
        return None
    ...
    time.sleep(config.CPU_USAGE_INTERVAL)
    try:
        with open(config.PROC_STAT_PATH) as f:
            lines2 = f.readlines()
    except FileNotFoundError:
...
def get_gpu_usage():
    gpu_usages = {}
    for card in os.listdir(config.DRM_ROOT):
        card_path = os.path.join(config.DRM_ROOT, card)
        try:
            vram_used_path = os.path.join(card_path, config.GPU_USED_VRAM_PATH)
            ...
            continue
        try:
            vram_total_path = os.path.join(card_path, config.GPU_TOTAL_VRAM_PATH)
...
def get_temp(sensor_name, temp_label):
    for hwmon in os.listdir(config.HWMON_ROOT):
        hwmon_path = os.path.join(config.HWMON_ROOT, hwmon)
        try:
